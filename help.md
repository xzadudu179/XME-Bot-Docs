...
- **指令列表：**
  ...
  ``` Text
  /takecoin (at目标用户) (星币数量 以空格分隔)
  ```

  - **权限/可用范围**

    在群聊内 & 是 SUPERUSER 或 是 bot 自己
...
- **作用**
  ...
  让{bot_name}帮忙决定事情吧！默认用空格分隔，若是英文则以英文逗号分隔。

  {bot_name}会因情况的不同而返回不同的结果，例如：在 choice 的参数中填写一个数字范围比如 `1~10` 会返回 1 ~ 10 的随机数

  其中，有一些可指定的全局变量，在choice里以{xxx}的方式使用即可调用：
